        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                if (sq.successors() != null) {
                    for (Place place : sq.successors()) {
                        Sq news = model.get(place);
                        if (sq.connectable(news)) {
                            nFound += 1;
                            found = news;
                            if (sq.sequenceNum() != 0
                                    && news.sequenceNum() != 0) {
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                if (sq.predecessors() != null) {
                    for (Place place : sq.predecessors()) {
                        Sq news = model.get(place);
                        if (news.connectable(sq)) {
                            nFound += 1;
                            found = news;
                            if (sq.sequenceNum() != 0
                                    && news.sequenceNum() != 0) {
