    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                elif insect.can_contain(self.ant) == True and self.ant.is_container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        bees_list = place.bees
        count = 0  
        while place != hive:      
            if bees_list != [] and self.min_range <= count <= self.max_range:
                return random_or_none(bees_list)
            else:  
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount == 0:
            bnew_list = self.place.bees[:]
            for bees in bnew_list:
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9 
        if self.contained_ant is not None and self.armor >0:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self, armor)
        QueenAnt.imposter_count += 1
        if QueenAnt.imposter_count > 1:
            self.imposter = True 
        ...
        while behind is not None:
            if behind.ant is not None:
                if behind.ant.is_buffed == False:
                    behind.ant.damage = behind.ant.damage * 2
                    behind.ant.is_buffed = True   
                if behind.ant.is_container == True and behind.ant.contained_ant is not None: 
                    if behind.ant.contained_ant.is_buffed == False:
                        behind.ant.contained_ant.damage = behind.ant.contained_ant.damage * 2
                        behind.ant.contained_ant.is_buffed = True
...
def make_scare(action, bee):
    ...
    def action_scare(colony):   
        bee.is_forward = False
        action(colony)
...
def apply_effect(effect, bee, duration):
    ...
    count = 0 
    affect_action = effect(bee.action, bee)
