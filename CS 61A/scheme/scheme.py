...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args_ = operands.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(operator, args_, env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError ('Number of arguments does not equal number of formal parameters')
        else: 
